...
let send = ((param,s) : (message ,storage)) : return =>
{
    ...
    if (Set.mem (Tezos.sender, s.authorized_addresses) == false) { 
        ...
        if (Set.cardinal (new_voters) >= s.threshold) {
          ...
          let increment = ((acc,addr): (reputation, address)) : reputation => switch (Map.find_opt (addr, acc)) {
            | Some(count) => Map.update(addr, Some(count + 1n), acc)
            | None => Map.add(addr, 1n, acc)
            };
          let reputation_updated : reputation = Set.fold (increment, new_voters, s.reputation);
