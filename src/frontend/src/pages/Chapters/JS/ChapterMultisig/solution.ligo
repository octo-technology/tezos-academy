...
const send = (param: message, s: storage) : return_ => {
    ...
    if (Set.size(new_voters) >= s.threshold) { 
        ...
        const increment = (acc: reputation, addr: address) : reputation => match(Map.find_opt(addr, acc), {
            Some: (count: nat) => Map.update(addr, Some(count + (1 as nat)), acc),
            None: () => Map.add(addr, 1 as nat,  acc)
        });
        const reputation_updated : reputation = Set.fold(increment, new_voters, s.reputation);
        return [ ret_ops, {
            ...s, 
